...
# Open report file
report = open('Report_training_' + model_name + '.txt', 'w')
...
for e in range(1, n_ensembles+1):
    ...
    # Data loaders
    train_loader = get_dataloader_pairs(df, train_index, target, graph_column_solvent, graph_column_solute, batch_size, shuffle=True, drop_last=False)
    valid_loader = get_dataloader_pairs(df, valid_index, target, graph_column_solvent, graph_column_solute, batch_size, shuffle=False, drop_last=False)
    test_loader  = get_dataloader_pairs(df, test_index, target, graph_column_solvent, graph_column_solute, batch_size,  shuffle=False, drop_last=False)
...
df_split.to_csv('Split_'+model_name+'.csv', index=False) # Save splits
report.close()